...
def delete():
    username = entryName.get()
    temp_passwords = []
    user_exists = False  # Flag to check if the user exists
    
    try:
        # Read the file and check if the username exists
        with open("passwords.txt", 'r') as f:
            for k in f:
                i = k.split(' ')
                if i[0] == username:
                    user_exists = True  # Username found
                else:
                    temp_passwords.append(f"{i[0]} {i[1]}")  # Keep other entries

        if user_exists:
            # If the user exists, write back the other users to the file
            ...
            messagebox.showerror("Error", f"No such username found")
...
if __name__ == "__main__":
    ...
    # List Button
    buttonList = tkinter.Button(app, text="List", command=display)
